...
token extlang { [ '-' <.alpha> ** 3 <?after $||'-'> ] ** 0..3 }

token script        { <.alpha>    ** 4                                 }
token region        { <.alpha>    ** 2    || <.digit>             ** 3 }
token variant       { <.alphanum> ** 5..8 || <.digit> <.alphanum> ** 3 }
token extension     { <singleton> ['-' (<.alphanum> ** 2..8)]+ }
token privateuse    { <[xX]>                 ['-' <privateusetag>] +   }
token privateusetag { <.alphanum> ** 1..8                              }
token grandfathered { <.irregular>         | <.regular>                }

#| New grandfathered tags will never be added, so they are hardcoded
token irregular {
